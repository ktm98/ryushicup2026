...
- 上皮組織は前立腺がんの発生母地であり、診断（異型・構造パターン、グリーソン評価、浸潤範囲）に直結するため主ターゲット。
- 3D蛍光病理（核DNA + 細胞質タンパク質の2チャンネル）をH&Eライクな偽色へ変換し、病理医の視覚に近い入力にしている。
- 3Dスキャンは非破壊で再利用可能、従来の2D切片では失われる3D文脈を保持できる。
- 巨大3Dボリューム（1検体あたり数十GB規模）から、以下のパイプラインで競技データを作成。
  - Step1: 巨大3Dボリュームから開始。
  - Step2: XZ平面に沿って一定間隔（5px）でスライス。
  - Step3: 学習用3Dブロックを切り出し（320×320ピクセル、奥行き64スライス）。
  - Step4: 2D画像として配布（1スライス＝1画像）。
- データリーク防止のため患者単位で分割。
- 目的は「上皮組織の精密セグメンテーション」。提出はRLEで、学習時はマルチクラスだが評価は上皮（クラス2）のみ。
- 3D全体像は非常に大きく（例: 高さ・幅・奥行きが数千〜1万ピクセル級）、2D化によって計算負荷を抑えつつも3D由来の文脈を反映できるように設計されている。
- 蛍光2チャンネルは核（DNA）と細胞質（タンパク質）に相当し、H&E風の色割り当てで可視化される。
- 2D画像はトレーニング/テストに分割され、提出は2DマスクをRLEで圧縮する形式。
